...
// Path data: Twemoji's handshake glyph (CC BY 4.0, twitter/twemoji), recolored
// to the app's brass/brass-dim palette. Split into its two natural halves —
// the emoji's own source art already separates into a "back" arm (darker
// tone) and a "front" arm (lighter tone), which happens to be exactly the
// two pieces needed to animate as separate hands.
const LEFT_ARM =
...
export function HandshakeIntro() {
  const reduceMotion = useReducedMotion();
  if (reduceMotion) return null; // skip straight to the photo reveal

  return (
    <motion.div
      aria-hidden
      className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center rounded-full bg-surface"
      animate={{ opacity: [1, 1, 0] }}
      transition={{ duration: 0.95, times: [0, 0.82, 1], ease: "easeInOut" }}
    >
      <motion.svg
        viewBox="0 0 36 36"
        className="h-16 w-16"
        animate={{ y: [0, 0, -2.5, 2, -1.5, 0] }}
        transition={{ duration: 0.35, delay: 0.45, times: [0, 0.15, 0.4, 0.62, 0.85, 1], ease: "easeInOut" }}
      >
        <motion.g
          initial={{ x: -13, y: 7, opacity: 0 }}
          animate={{ x: 0, y: 0, opacity: 1 }}
          transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}
        >
          <path fill="#8A7346" d={LEFT_ARM} />
          <path fill="#8A7346" d={LEFT_DETAIL} />
        </motion.g>
        <motion.g
          initial={{ x: 13, y: 7, opacity: 0 }}
          animate={{ x: 0, y: 0, opacity: 1 }}
          transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}
        >
          <path fill="#C9A15C" d={RIGHT_ARM} />
          <path fill="#C9A15C" d={RIGHT_DETAIL} />
        </motion.g>
      </motion.svg>
    </motion.div>
  );
